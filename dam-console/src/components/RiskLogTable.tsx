...
import { Download, ChevronRight, ChevronDown, Activity, Shield, Hash, Clock, Play, Pause, Trash2 } from 'lucide-react'
      ...
      {open && (
        <div className="border-t border-dam-border/40 px-3 pb-2 pt-1.5 space-y-1.5 bg-black/20">
          {g.reason ? (
            ...
            <p className="text-[11px] text-dam-muted/50 italic">Safety check passed — no telemetry reported.</p>
          ) : null}
          {g.metadata && Object.keys(g.metadata).length > 0 && (
...
function GuardMetadataView({ metadata }: { metadata: Record<string, unknown> }) {
  const exceptionJoints = new Set(
    Array.isArray(metadata.exception_joints) ? (metadata.exception_joints as number[]) : []
  )
  const sections: { key: string; label: string; unit: string; format: (v: number) => string }[] = [
    { key: 'temperatures', label: 'Temperature', unit: '°C', format: v => v.toFixed(0) + '°' },
    { key: 'currents',     label: 'Current',     unit: 'A',  format: v => v.toFixed(2) + 'A' },
    { key: 'voltages',     label: 'Voltage',     unit: 'V',  format: v => v.toFixed(1) + 'V' },
  ]
  const rows = sections
    .map(s => {
    ...
    .filter(Boolean) as { key: string; label: string; entries: [string, number][]; format: (v: number) => string }[]
    ...
    <div className="space-y-1.5 font-mono">
      {rows.map(r => (
        <div key={r.key} className="text-[10px]">
          <div className="text-dam-muted/70 uppercase tracking-wider mb-0.5">{r.label}</div>
            ...
            {r.entries.map(([joint, val]) => {
              ...
              return (
                <span
                  ...
                  title={skipped ? `${joint} skipped by exception_joints` : joint}
                >
      ...
      {exceptionJoints.size > 0 && (
        <p className="text-[10px] text-dam-muted/60 italic pt-0.5">
          Skipped joints: {[...exceptionJoints].sort((a, b) => a - b).map(i => `J${i}`).join(', ')}
        </p>
      )}
    </div>
  )
...
function pickNumericDict(meta: Record<string, unknown> | undefined, key: string): [string, number][] | null {
  const v = meta?.[key]
...
function sensorGlimpse(events: RiskEvent[]): { tMax?: [string, number]; iMax?: [string, number] } {
  // Latest event with hardware metadata; fall back to scanning if topmost lacks it.
  const meta = events.find(e =>
    e.guard_results?.some(g => g.metadata && (g.metadata.temperatures || g.metadata.currents)),
  )
  if (!meta) return {}
  const hw = meta.guard_results.find(g => g.metadata?.temperatures || g.metadata?.currents)?.metadata
  const temps = pickNumericDict(hw, 'temperatures')
  const currs = pickNumericDict(hw, 'currents')
  const tMax = temps?.reduce<[string, number]>((acc, e) => (e[1] > acc[1] ? e : acc), temps[0])
  const iMax = currs?.reduce<[string, number]>((acc, e) => (e[1] > acc[1] ? e : acc), currs[0])
  return { tMax, iMax }
}
...
const _DECISION_RANK: Record<string, number> = { PASS: 0, CLAMP: 1, REJECT: 2, FAULT: 3 }
...
function GuardChips({ results }: { results: GuardStatus[] }) {
  ...
  for (const g of results) {
    ...
    if (!slot) {
      byLayer.set(g.layer, { worst: g.decision, reasons: g.reason ? [`${g.name}: ${g.reason}`] : [] })
      continue
    }
  ...
  const layers = [...byLayer.entries()].sort((a, b) => a[0].localeCompare(b[0]))

  const colorFor = (d: string) =>
    d === 'PASS'                       ? 'bg-emerald-950/40 text-dam-green border-emerald-900/40' :
    d === 'CLAMP'                      ? 'bg-blue-950/40 text-dam-blue border-blue-900/40'        :
    ...
    <div className="flex flex-wrap gap-1">
      {layers.map(([layer, info]) => (
        <span
          ...
          title={info.reasons.length > 0 ? info.reasons.join('\n') : `${layer} ${info.worst}`}
        >
          {layer}
        </span>
      ))}
...
export function RiskLogTable() {
  ...
  const [expandedAnchorIds, setExpandedAnchorIds] = useState<Set<number>>(new Set())
  ...
  const groupEvents = (raw: RiskEvent[]) => {
    if (raw.length === 0) return [] as (RiskEvent & { count: number; events: RiskEvent[] })[]
  ...
  useEffect(() => {
    ...
    const g = groups.find(grp => grp.events.some(e => e.event_id === ev.event_id))
    if (g) {
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-dam-border/40">
            {groupedEvents.length === 0 ? (
              <tr><td colSpan={6} className="text-center text-dam-muted py-12">No risk events recorded.</td></tr>
            ) : groupedEvents.map(e => {
              ...
              const isExpanded = e.events.some(s => expandedAnchorIds.has(s.event_id))
              const isTarget = targetCycleId !== null && e.events.some(s => s.cycle_id === targetCycleId)
                ...
                <React.Fragment key={reactKey}>
                  ...
                  >
                    <td className="px-4 py-3 text-center text-dam-muted">
                      <div className="flex items-center gap-1.5">
                        ...
                        {e.count > 1 && (
                          <span className="text-[9px] text-dam-blue font-mono font-bold">×{e.count}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        {e.fallback_triggered && <span className="px-1.5 py-0.5 rounded bg-orange-950/40 text-dam-orange border border-orange-900/40 font-bold text-[10px]">FALLBACK</span>}
                        {!e.was_rejected && !e.was_clamped && !e.fallback_triggered && <span className="text-dam-muted">—</span>}
                      </div>
                    </td>
                  ...
                  {isExpanded && (
                    <tr className="bg-dam-surface-2/40 animate-in slide-in-from-top-1 duration-200">
                      <td colSpan={6} className="px-8 py-4 border-l-2 border-dam-blue">
                        {e.count === 1 ? (
                          ...
                          <div className="space-y-1.5">
                            <div className="flex items-center justify-between text-[10px] text-dam-muted uppercase tracking-widest pb-1">
                              <span>{e.count} cycles in group — click a row for full detail</span>
                              {(glimpse.tMax || glimpse.iMax) && (
                                <span className="normal-case tracking-normal font-mono">
                                  {glimpse.tMax && <span className="mr-3">max T: <span className="text-dam-text">{glimpse.tMax[0]} {glimpse.tMax[1].toFixed(0)}°</span></span>}
                                  {glimpse.iMax && <span>max I: <span className="text-dam-text">{glimpse.iMax[0]} {glimpse.iMax[1].toFixed(2)}A</span></span>}
                                </span>
                              )}
                            </div>
                            {e.events.map(sub => {
                              const innerOpen = expandedEventIds.has(sub.event_id)
                              const subGlimpse = sensorGlimpse([sub])
                              return (
                                <div key={sub.event_id} className="bg-dam-surface-3 border border-dam-border rounded">
                                  <button
                                    type="button"
                                    onClick={() => toggleEventExpand(sub.event_id)}
                                    className="w-full flex items-center gap-2 px-2 py-1.5 text-left hover:bg-dam-surface-2 transition-colors"
                                  >
                                    <span className="text-dam-muted/50 shrink-0">
                                      {innerOpen ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                                    </span>
                                    <span className="text-[10px] text-dam-muted font-mono w-24 shrink-0">{fmtTimeMs(sub.timestamp)}</span>
                                    <span className="text-[10px] text-dam-muted font-mono w-14 shrink-0">#{sub.event_id}</span>
                                    <span className="text-[10px] text-dam-muted font-mono w-20 shrink-0">cyc {sub.cycle_id}</span>
                                    {sub.guard_results && sub.guard_results.length > 0 && (
                                      <span className="shrink-0"><GuardChips results={sub.guard_results} /></span>
                                    )}
                                    <span className="flex-1 text-[10px] font-mono text-dam-muted truncate">
                                      {subGlimpse.tMax && <span className="mr-2">{subGlimpse.tMax[0]}:{subGlimpse.tMax[1].toFixed(0)}°</span>}
                                      {subGlimpse.iMax && <span>{subGlimpse.iMax[0]}:{subGlimpse.iMax[1].toFixed(2)}A</span>}
                                    </span>
                                    <span className="text-[10px] font-mono text-dam-muted shrink-0">{sub.latency_ms['total']?.toFixed(1) ?? '—'} ms</span>
                                  </button>
                                  {innerOpen && (
