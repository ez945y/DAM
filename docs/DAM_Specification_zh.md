...
### 3.1 明確驗證重於概率性輸出
神經網絡策略產生的是概率性的動作提案 (Action Proposal)。DAM 會在分發前依照明確的邊界規則驗證提案動作；框架不僅依賴模型信心度。
...
DAM 支持三種**運行時執行模式**和**具名守衛配置文件**，以在不更改守衛程式碼的情況下涵蓋訓練、評估和受監督硬體部署。

**執行模式：**

| 模式 | 守衛管線 | 動作分發 | 用例 |
| :--- | :--- | :--- | :--- |
| `enforce` | 完整驗證 | `ValidatedAction`（限制或拒絕）；可觸發 fallback | 受監督硬體部署 |
