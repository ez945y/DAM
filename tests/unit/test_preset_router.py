...
def test_create_rejects_empty_name(client):
    resp = client.post(
        "/api/system/presets",
        json={"name": "", "joint_names": ["j"], "degrees_mode": True, "urdf_path": None},
    )
...
def test_create_rejects_empty_joints(client):
    resp = client.post(
        "/api/system/presets",
        json={"name": "foo", "joint_names": [], "degrees_mode": True, "urdf_path": None},
    )
    assert resp.status_code == 400
...
def test_create_rejects_non_list_joints(client):
    resp = client.post(
        "/api/system/presets",
        json={"name": "foo", "joint_names": "not_a_list", "degrees_mode": True, "urdf_path": None},
    )
    assert resp.status_code == 400
