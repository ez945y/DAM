...
impl SerializerBus {
    ...
    fn build_cycle_message(record: &serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        Ok(serde_json::json!({
            ...
            "active_context": record.get("active_context").cloned().unwrap_or(serde_json::json!("normal")),
            "context_severity": record.get("context_severity").cloned().unwrap_or(serde_json::json!(0)),
            "context_event": record.get("context_event").cloned().unwrap_or(serde_json::Value::Null),
    ...
    fn build_guard_result_message(
        ...
        Ok(serde_json::json!({
            ...
            "event_class": result.get("event_class").cloned().unwrap_or(serde_json::Value::Null),
