...
def create_risk_log_router(
    ...
    def _resolve_mcap(request: Request) -> Any:
    ...
    async def risk_log_from_mcap(
        ...
        """Lightweight listing built from the cycle index — no per-cycle MCAP reads."""
        svc = _resolve_mcap(request)
        if svc is None:
            raise HTTPException(503, _MCAP_UNAVAILABLE)
        events: list[dict[str, Any]] = []
        for cycle in reversed(svc.list_cycles(filename)):
            if not cycle.get("has_violation") and not cycle.get("has_clamp"):
                continue
            resolved_outcome = "reject" if cycle.get("has_violation") else "clamp"
            risk_level = "CRITICAL" if resolved_outcome == "reject" else "ELEVATED"
            if outcome and resolved_outcome != outcome:
                continue
            if failure_type and cycle.get("failure_type") != failure_type:
                continue
            if min_risk_level and _RISK_RANK.get(risk_level, 0) < _RISK_RANK.get(min_risk_level, 0):
                continue
            total_ms = cycle.get("total_ms", 0.0)
            events.append(
                {
                    "event_id": -(int(cycle["cycle_id"]) + 1),
                    "timestamp": cycle.get("timestamp", 0.0),
                    "cycle_id": cycle["cycle_id"],
                    "trace_id": f"mcap:{filename}:{cycle['cycle_id']}",
                    "risk_level": risk_level,
                    "outcome": resolved_outcome,
                    "was_clamped": bool(cycle.get("has_clamp")),
                    "was_rejected": bool(cycle.get("has_violation")),
                    "fallback_triggered": None,
                    "guard_results": [],
                    "latency_ms": {"total": total_ms},
                    "perf": None,
                    "mcap_filename": filename,
                    "failure_type": cycle.get("failure_type"),
                }
            )
            if len(events) >= limit:
                break
    ...
    @router.get(
        "/mcap/{filename}/cycle/{cycle_id}",
        responses={503: {"description": _MCAP_UNAVAILABLE}},
    )
    async def risk_log_mcap_cycle_detail(
        request: Request,
        filename: Annotated[str, Path()],
        cycle_id: Annotated[int, Path()],
    ) -> Response:
        """Full detail for a single MCAP cycle — called on expand."""
        svc = _resolve_mcap(request)
        if svc is None:
            raise HTTPException(503, _MCAP_UNAVAILABLE)
        detail = svc.get_cycle_detail(filename, cycle_id)
        if detail is None:
        ...
        layers = {k: latency_raw.get(k, 0.0) for k in ("L0", "L1", "L2", "L3") if k in latency_raw}
        perf = {"stages": stages, "layers": layers, "guards": per_guard_latency} if stages else None
        ...
        event = {
            "event_id": -(cycle_id + 1),
            "timestamp": detail.get("timestamp", 0.0),
            "cycle_id": cycle_id,
            "trace_id": f"mcap:{filename}:{cycle_id}",
            "risk_level": risk_level,
            "outcome": resolved_outcome,
        ...
        return MsgspecJSONResponse(event)
